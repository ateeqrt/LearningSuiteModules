var lanMod = 'rt_Tracker';
window.websocket = function (interval) {
	if (typeof window.zfid == 'undefined' || _.isEmpty(window.zfid)) {
		...
		if ("WebSocket" in window) {
			...
			ws.onmessage = function (evt) {
				...
				if (msg.type == 'status') {
					...
					var prefsURL = App.api.buildURL('rtCXM/recurUser/' + window.btoa(JSON.stringify(msg.sfrom)), null, null, {
					...
					App.api.call('POST', prefsURL, null, {
						success: _.bind(function (data) {
							...
							page_c = data.page_c;
							if (data.notify == '' || data.notify === App.user.id) {
								if (data.visits > 2) {
									$.notify(App.lang.get('LBL_SUGGEST_CHAT1', lanMod) + "'" + msg.name + "'. " + App.lang.get('LBL_SUGGEST_CHAT2', lanMod) + "'" + data.potential_interest + "'", {
										position: "top right",
										autoHideDelay: 10000,
									...
									$.notify(App.lang.get('LBL_RET_USER', lanMod), {
										position: "top right",
										style: 'rtcxmchatstyle'
									});
							}
					...
					if ($('#nconvo').is(':visible')) {
						if ($('.heading').attr('id')) {
							...
							if (msg.sfrom == id) {
								...
								$('.messages').notify({title: '<p>' + App.lang.get('LBL_1_NM', lanMod) + '</p>'}, {
									style: 'newmessage',
									autoHide: false,
									position: "top left"
								});
						...
						$.notify(App.lang.get('LBL_NEW_MESSAGE', lanMod) + msg.name, {
							position: "top right",
							style: 'rtcxmchatstyle'
						});
					}
